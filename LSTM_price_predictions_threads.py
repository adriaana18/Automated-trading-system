...
import os
import numpy as np
import tensorflow as tf # This code has been tested with TensorFlow 2.4.1
...
def calculate_predictions(csv_file):    
#%%
    """
    DATA PREPROCESSING
    """
    ...
    """
    xlbl = csv_file + ' Date'
    ylbl = csv_file + ' Mid Price'
    lbl = csv_file + ' Stock Price'
    plt.figure(figsize = (30,15))
    plt.plot(range(df.shape[0]),(df['PX_LOW']+df['PX_HIGH'])/2.0, label = lbl)
    ...
    for ep in range(epochs):       
            ...
            if (ep+1) % valid_summary == 0:
              ...
              # The average loss
              if (ep+1)%valid_summary==0:
                print('%s Average loss at step %d: %f' % (csv_file, ep+1, average_loss))
              ...
              if loss_nondecrease_count > loss_nondecrease_threshold :
                    ...
                    loss_nondecrease_count = 0
                    print('\tDecreasing learning rate by 0.5 for ' + csv_file)
        
              test_mse_ot.append(current_test_mse)
              print('\tTest MSE: %.5f'%np.mean(mse_test_loss_seq))
              predictions_over_time.append(predictions_seq)
              print('\tFinished Predictions for ' + csv_file)
    ...
    #%%
    """
    Use price predictions to create return predictions
    """
    ...
    predictions_df.to_excel(excel_writer = "C:/Users/adria/OneDrive/Documents/PPE/EC331_RAE/Code/{}.xlsx".format())
